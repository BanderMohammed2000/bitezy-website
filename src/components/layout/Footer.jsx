    ...
    <section id="contact" className="bg-bgCream">
      <div className="container mx-auto pt-12 pb-6 px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-20">
          ...
          {[aboutUsList, ourMenuList, ourLocationList].map(
            (list, listIndex) => (
              <motion.div
                key={listIndex}
                initial={{ opacity: 0, y: 22 }}
              ...
              >
                ...
                <ul className="space-y-2">
                  {list.map((data, index) => (
                    <li
                      key={index}
                      className="text-graySoft text-sm capitalize"
                    >
                      {data}
                    </li>
                  ))}
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.45 }}
          transition={{ duration: 0.9, ease: smoothEase, delay: 0.45 }}
          className="flex justify-center mt-20"
        >
          <p className="text-graySoft text-sm font-medium capitalize">
            copyright &copy; {currentYear} the bitezy
          </p>
        </motion.div>
      </div>
    </section>
